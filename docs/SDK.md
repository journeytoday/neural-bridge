...
`engine.installOrder(order)` installs a validated test constraint; later apply and undo enforce its minimum dwell, and expired orders fail closed. The test checksum remains non-cryptographic. `reviewExpiredOrder()` requires explicit demo-user review after expiry; ordinary free text remains available. `exportMemory/importMemory` persist text-free summaries and refusals, never live calibration or reliability. `voiceContribution` gates a transcript on qualified recent evidence; explicit corrected text remains available independently.
